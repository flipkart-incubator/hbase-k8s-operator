...
func (r *HbaseClusterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	svc := buildService(hbasecluster.Name, hbasecluster.Name, hbasecluster.Namespace, deployments, true)
	...
	for _, namespace := range namespaces {
		cfg := buildConfigMap(hbasecluster.Spec.Configuration.HbaseConfigName, hbasecluster.Name, namespace, hbasecluster.Spec.Configuration.HbaseConfig)
		...
		cfg = buildConfigMap(hbasecluster.Spec.Configuration.HadoopConfigName, hbasecluster.Name, namespace, hbasecluster.Spec.Configuration.HadoopConfig)
		ctrl.SetControllerReference(hbasecluster, cfg, r.Scheme)
		result, err = reconcileConfigMap(ctx, log, namespace, cfg, r.Client)
		if (ctrl.Result{}) != result || err != nil {
			return result, err
	...
	for _, d := range deployments {
		...
		if d.IsPodServiceRequired {
			...
			for index < d.Size {
				...
				svc = buildService(name, hbasecluster.Name, hbasecluster.Namespace, []kvstorev1.HbaseClusterDeployment{d}, false)
				ctrl.SetControllerReference(hbasecluster, svc, r.Scheme)
				result, err = reconcileService(ctx, log, hbasecluster.Namespace, svc, r.Client)
				if (ctrl.Result{}) != result || err != nil {
					return result, err
