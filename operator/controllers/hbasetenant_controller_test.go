...
func TestHbaseTenantReconciler_ResNotFound(t *testing.T) {
	mockClient, reconciler := getMockClientAndTenantReconciler()
	ctx := context.TODO()
	req := ctrl.Request{
...
func TestHbaseTenantReconciler_SuccessfulReconciliation_ObjectsNotFound(t *testing.T) {
	...
	mockClient.On("Get", ctx, req.NamespacedName, &kvstorev1.HbaseTenant{}).
	...
	mockClient.On("Get", ctx, types.NamespacedName{Name: hbasetenant.Spec.Datanode.Name, Namespace: hbasetenant.Namespace}, &appsv1.StatefulSet{}).Return(errors.NewNotFound(schema.GroupResource{}, req.Name))
	...
	mockClient.On("Get", ctx, types.NamespacedName{Name: mockStsSvc.Name, Namespace: hbasetenant.Namespace}, &corev1.Service{}).Return(errors.NewNotFound(schema.GroupResource{}, req.Name))
	mockClient.On("Create", ctx, mockStsSvc, []client.CreateOption(nil)).Return(nil)
...
func TestHbaseTenantReconciler_SuccessfulReconciliation_AllObjectsFound(t *testing.T) {
	...
	mockStsSvc := buildService(hbasetenant.Name, hbasetenant.Name, hbasetenant.Namespace, hbasetenant.Spec.ServiceLabels, hbasetenant.Spec.ServiceSelectorLabels, []kvstorev1.HbaseClusterDeployment{hbasetenant.Spec.Datanode}, true)
	ctrl.SetControllerReference(hbasetenant, mockStsSvc, reconciler.Scheme)
	mockClient.On("Get", ctx, types.NamespacedName{Name: mockStsSvc.Name, Namespace: hbasetenant.Namespace}, &corev1.Service{}).
		Run(func(args mock.Arguments) {
			arg := args.Get(2).(*corev1.Service)
			*arg = *mockStsSvc
		}).
		Return(nil)
	mockClient.On("Update", ctx, mockStsSvc, []client.UpdateOption(nil)).Return(nil)
	...
	mockClient.On("Get", ctx, types.NamespacedName{Name: hbasetenant.Spec.Datanode.Name, Namespace: hbasetenant.Namespace}, &appsv1.StatefulSet{}).
		Run(func(args mock.Arguments) {
			arg := args.Get(2).(*appsv1.StatefulSet)
			*arg = *mockSts
		}).Return(nil).Times(2)

	//mockClient.On("Update", ctx, mockSts, []client.UpdateOption(nil)).Return(nil)
...
func TestHbaseTenantReconciler_SuccessfulReconciliation_AllObjectsFoundRestFlow(t *testing.T) {
	...
	mockClient.On("Get", ctx, types.NamespacedName{Name: hbasetenant.Spec.Datanode.Name + "-pdb", Namespace: hbasetenant.Namespace}, &policyv1.PodDisruptionBudget{}).Return(errors.NewNotFound(schema.GroupResource{}, req.Name))
	mockClient.On("Create", ctx, mockPdb, []client.CreateOption(nil)).Return(nil)
...
func getMockClientAndTenantReconciler() (*MockClient, *HbaseTenantReconciler) {
...
func getInvalidConfigHbasetenant() *kvstorev1.HbaseTenant {
	tenant := &kvstorev1.HbaseTenant{}
	hbaseTenantJson := "{\"apiVersion\":\"kvstore.abc.com/v1\",\"kind\":\"HbaseTenant\",\"metadata\":{\"labels\":{\"app.kubernetes.io/managed-by\":\"Helm\"},\"name\":\"yak-tenant-test-1\",\"namespace\":\"yak-tenant-test-1-ns\"},\"spec\":{\"baseImage\":\"test-image\",\"configuration\":{\"hadoopConfig\":{\"core-site.xml\":\"<?xmlversion=\\\"1.0\\\"?>\\n<?xml-stylesheettype=\\\"text/xsl\\\"href=\\\"configuration.xsl\\\"?>\\n<!--Generatedbyconfdon2021-03-0911:46:01.973761409+0530ISTm=+0.012850605-->\\n<configuration>\\n<property>\\n<name>fs.trash.interval</name>\\n<value>1440</value>\\n</property>\\n\"}},\"serviceLabels\":{\"hbase-operator.cfg-statefulset-update/enable\":\"config-only\"}}}"
	err := json.Unmarshal([]byte(hbaseTenantJson), tenant)
	if err != nil {
		fmt.Println(err)
	}
