...
func getStatefulSetAnnotation(log logr.Logger, cl client.Client, ctx context.Context, statefulSetName string, namespace string) string {
	...
	resourceAnnotation, exists := existingStatefulSet.Spec.Template.Annotations[STATEFULSET_V2_ANNOTATION]
	if exists {
		lastStatefulSetConfigVersion = resourceAnnotation
	}
	return lastStatefulSetConfigVersion
}
...
func buildInitContainers(baseImage string, config kvstorev1.HbaseClusterConfiguration, cs []kvstorev1.HbaseClusterInitContainer, isBootstrap bool) []corev1.Container {
	...
	for _, c := range cs {
		...
		if !c.IsBootstrap || (isBootstrap && c.SpawnState == "default") || (!isBootstrap && c.SpawnState == "afterBootstrap") {
